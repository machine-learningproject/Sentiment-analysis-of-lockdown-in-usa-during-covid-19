...
# Twitter credentials
consumer_key = 'O3FgW51ERSKLHn0mkYjc20wTK'
consumer_secret = '4SspIPc0ifGDzOyysF0qCe5rRpjmYVd1F7Vlo8lNZ6t4bK3rFD'
access_key = '1576072534314795009-gxGxoF5ShxTD601ai6gN7tkJE06uWk'
access_secret = 'Hw1NNiITbSNguJ2h1A0OrHDLKABVE9i8DboYyeLAYv2Nf'
...
def extract_tweets(search_words, date_since, date_until, numTweets):
    return (tweepy.Cursor(t_api.search, q=search_words, lang="en", since=date_since, until=date_until,
...
def scraptweets(search_words, date_since, date_until, numTweets, numRuns):
    ...
    db_tweets = pd.DataFrame(
        columns=['username', 'acctdesc', 'location', 'following', 'followers', 'totaltweets', 'usercreatedts',
    ...
    for i in range(numRuns):
        ...
        for tweet in tweet_list:
            ...
            itweet = [username, acctdesc, location, following, followers, totaltweets, usercreatedts, tweetcreatedts,
                      retweetcount, text, lst]
...
def generate_wordcloud(data, title):
    wc = WordCloud(width=400, height=330, max_words=150, colormap="Dark2", background_color='white',
...
def plotGenerator():
    ...
    while not thread_stop_event.isSet():
        ...
        search_words = "(#Usa AND #COVID-19) OR #COVID19Usa"
        ...
        for i in range(3):
            ...
            Word_frequency = pd.Series(' '.join(Data.selected_text).split()).value_counts()[
                             :20]  # Calculating the words frequency
        ...
        for i in range(3):
            ...
            data["selected_text"] = data["selected_text"].apply(
                lambda x: ' '.join([word for word in x.split() if word not in (eng_stopwords)]))
            Sentiment = Analysis_Data[
                Analysis_Data['sentiment'] == df.index[i]]  # Creating the dataframe of having same sentiment
            Word_frequency = pd.Series(' '.join(Sentiment.selected_text).split()).value_counts()[
                             :50]  # Calculating the words frequency
...
def gettweets():
    while not twitter_thread_stop_event.isSet():
        ...
        for tweet in tweepy.Cursor(t_api.search, q="#" + "COVID19India" + " -filter:retweets", rpp=5, lang="en",
            ...
            socketio.emit('tweets', {'Text': temp['text'], 'Username': temp['username'], 'Sentiment': temp['label']},
                          namespace='/twitter')
